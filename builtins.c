...
char **command_unsetenv(char **args, char **env) {
    ...
    int env_count = count_env_vars(env);
    char **new_env = malloc((env_count + 1) * sizeof(char *)); // +1 for NULL
    if (!new_env) {
        perror("malloc");
        return env;
    }
    ...
    for (int i = 0; i < env_count; i++) {
        if (my_strncmp(env[i], args[1], my_strlen(args[1])) == 0 &&
            env[i][my_strlen(args[1])] == '=') {
            found = 1;
            continue; // skip this variable (don’t copy)
        }
        new_env[j++] = my_strdup(env[i]); // copy safely
    }
    ...
    if (!found) {
        ...
        // free the new_env we allocated
        for (int k = 0; k < j; k++) free(new_env[k]);
        free(new_env);
        return env;
    }
